...
public class FetchTypeTest {
    ...
    @Test
    @Transactional // 지연로딩 읽어올 때 걸어줘야 한다!
    ...
    void test1() {
        ...
        // default 인 EAGER에서 LAZY로 변경해보았다!
        // 이 부분, join 필요할 때만 해당 구문 다시 호출한다 > 지연 로딩
    ...
    void test2() {
        User user = userRepository.findByName("Robbie"); // 동적으로 값을 가져오기 때문에 orElse 필요하지 않다
        System.out.println("user.getName() = " + user.getName());

        System.out.println("Robbie가 주문한 음식 이름 조회");
        for (Food food : user.getFoodList()) { // user 객체의 id를 가지고 food 테이블을 조회한다.
            System.out.println(food.getName());
        }

        // 지연 로딩 > SQL 문제어 join 던지지 않는다!
    }

    @Test
    @DisplayName("Robbie 고객 조회 실패") // 지연 로딩인데 Transaction 없을 때
    void test3() {
    ...
    void init() {
        ...
        // 지금까지 추가한 Entity 타입의 비영속화 상태의 List를 담는다.
